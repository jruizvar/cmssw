...
//
// JB Sauvan (23/10/2013): L1CaloClusterWithSeedEGIsolator is a copy of L1CaloClusterEGIsolator using different type of clusters (L1CaloClusterWithSeed).
// This class is used for the new e/g clustering. It is not a good idea to
// duplicate the class like this, but it is the fastest way. Anyway,
// everything should be reorganised.
// 

class L1CaloClusterWithSeedEGIsolator : public edm::EDProducer {
    private:
        edm::InputTag caloClustersTag_;
        edm::InputTag caloTowersTag_;
        edm::InputTag rhoTag_;

        edm::Handle<l1slhc::L1CaloClusterWithSeedCollection> caloClustersHandle_;
    ...
    public:
        L1CaloClusterWithSeedEGIsolator(const edm::ParameterSet & );
        ~L1CaloClusterWithSeedEGIsolator();

        void algorithm(const int &, const int &,l1slhc::L1CaloClusterWithSeedCollection&);
...
L1CaloClusterWithSeedEGIsolator::L1CaloClusterWithSeedEGIsolator(const edm::ParameterSet & config):
    phiOffset_(0),
    etaOffset_(-1),
    phiIncrement_(1),
    etaIncrement_(1)
{
    ...
    produces<l1slhc::L1CaloClusterWithSeedCollection>();
}


L1CaloClusterWithSeedEGIsolator::~L1CaloClusterWithSeedEGIsolator(  )
{
}

void L1CaloClusterWithSeedEGIsolator::produce(edm::Event & iEvent, const edm::EventSetup & iSetup)
{
    ...
    std::auto_ptr<l1slhc::L1CaloClusterWithSeedCollection> outputCollection(new l1slhc::L1CaloClusterWithSeedCollection);
...
void L1CaloClusterWithSeedEGIsolator::algorithm( const int &trigEta, const int &trigPhi,l1slhc::L1CaloClusterWithSeedCollection& outputColl)
{

    //  std::cout <<"aEta "<<aEta <<" aPhi "<<aPhi<<std::endl;
    l1slhc::L1CaloClusterWithSeedCollection::const_iterator clusIt = getObject(trigEta,trigPhi,caloClustersHandle_);
    if(clusIt!=caloClustersHandle_->end()) { 

        // No "non-central" clusters for the new e/g clustering
        //if(clusIt->isCentral()){
...
int L1CaloClusterWithSeedEGIsolator::egEcalIsolation(int iEta,int iPhi)const
{
    ...
    // The leading tower is automatically at iEta,iPhi; so the loop is not needed.
    // But it's harmless, so keep it for the moment
...
template<class T> typename T::const_iterator L1CaloClusterWithSeedEGIsolator::getObject(const int& trigEta,const int &trigPhi,const edm::Handle<T>& handle)const
{
...
DEFINE_EDM_PLUGIN(edm::MakerPluginFactory,edm::WorkerMaker<L1CaloClusterWithSeedEGIsolator>,"L1CaloClusterWithSeedEGIsolator");
DEFINE_FWK_PSET_DESC_FILLER(L1CaloClusterWithSeedEGIsolator);
